...
const likeRestaurant = async (req, res, next) => {
  try {
    ...
    if (likes.length > 0) {
      return res
        .status(400)
        .json(error("Người dùng đã thích nhà hàng này rồi"));
    }

    // Thực hiện thêm lượt thích
    await db.query("INSERT INTO like_res (user_id, res_id) VALUES (?, ?)", [
      userId,
      resId,
    ]);
...
const unlikeRestaurant = async (req, res, next) => {
  try {
    ...
    if (likes.length === 0) {
      return res
        .status(404)
        .json(error("Thông tin thích nhà hàng không tồn tại"));
    }

    // Thực hiện xóa lượt thích
    await db.query("DELETE FROM like_res WHERE user_id = ? AND res_id = ?", [
      userId,
      resId,
    ]);
...
const getLikesByRestaurant = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(
        success("Lấy danh sách người dùng thích nhà hàng thành công", rows),
      );
  } catch (err) {
    next(err);
  }
...
const getLikesByUser = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(
        success("Lấy danh sách nhà hàng người dùng thích thành công", rows),
      );
  } catch (err) {
    next(err);
  }
